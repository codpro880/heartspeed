...
public:
    BobsTavern(Player*);
    std::vector<std::string> refresh_minions(Player*, bool is_free = false); // Reduce players gold by one
    std::vector<std::string> get_current_minions(); // Player arg not necessary for now, but probably will be in future
    void buy_minion(Player*, std::string minion); // Reduces players gold by three, adds minion to hand
    void buy_minion(Player*, int pos); // Reduces players gold by three, adds minion to hand
    void sell_minion(Player*, int board_bos); // Removes card from board, gives player a gold
    bool can_tavern_up(Player*, int turns_at_current_tier); // Level up tavern tier
    bool tavern_up(Player*, int turns_at_current_tier); // Level up tavern tier
